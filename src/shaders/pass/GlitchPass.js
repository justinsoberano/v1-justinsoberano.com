...
class GlitchPass extends Pass {
	...
	render( renderer, writeBuffer, readBuffer /*, deltaTime, maskActive */ ) {
		...
		if ( this.curF % this.randX == 0 || this.goWild == true ) {
			...
			this.uniforms[ 'seed_y' ].value = MathUtils.randFloat( 0, 0.01 );
			this.uniforms[ 'distortion_x' ].value = MathUtils.randFloat( 0, 0.01 );
			this.uniforms[ 'distortion_y' ].value = MathUtils.randFloat( 0, 0.01 );
			this.curF = 0;
			this.generateTrigger();
		} else if ( this.curF % this.randX < this.randX / 5 ) {
			this.uniforms[ 'amount' ].value = Math.random() / 1000; // 90
			this.uniforms[ 'angle' ].value = MathUtils.randFloat( 0, 1 );
		...
		this.curF += MathUtils.randFloat(0.00000001, 0.01); // ++
	...
	generateTrigger() {
		this.randX = MathUtils.randInt( 1, 2 ); // 240, 120
	}
	generateHeightmap( dt_size ) {
		const data_arr = new Float32Array( dt_size * dt_size );
		const length = dt_size * dt_size;
		for ( let i = 0; i < length; i ++ ) {
			const val = MathUtils.randFloat( 0, 2 );
			data_arr[ i ] = val;
		}
